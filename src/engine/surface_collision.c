...
// This function is used to figure out where to spawn red coins in autogenerated dungeons
f32 find_red_coin_zone_floor(s32 xPos, s32 yPos, s32 zPos, struct Surface **pfloor) {
    ...
    // Check for surfaces that are a part of level geometry.
    struct SurfaceNode *surfaceList = gStaticSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_RED_COIN_ZONE];
    struct Surface     *floor       = find_water_floor_from_list(surfaceList, x, y, z, &height);
