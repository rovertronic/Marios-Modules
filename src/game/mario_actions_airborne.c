...
void play_knockback_sound(struct MarioState *m) {
    if (m->actionArg == 0 && (m->forwardVel <= -28.0f || m->forwardVel >= 28.0f)) {
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_DOH), MARIO_MARIO_SOUND_PLAYED);
    } else {
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_UH), MARIO_MARIO_SOUND_PLAYED);
    }
}
...
s32 act_double_jump(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, get_mario_sound_id(MARIO_SND_HOOHOO));
...
s32 act_backflip(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, get_mario_sound_id(MARIO_SND_YAH_WAH_HOO));
...
s32 act_long_jump(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, get_mario_sound_id(MARIO_SND_YAHOO));
...
s32 act_twirling(struct MarioState *m) {
    ...
    if (element && m->pos[1] > element->object->oPosY) {
        ...
        if (x < 700.0f) {
            f32 force = 10.0f * sqrtf( (-x+700.0f)/700.0f  );
            if (force > 0.0f) {
                gMarioState->vel[1] += force;
            }
        }
    }
    */
...
s32 act_dive(struct MarioState *m) {
    if (m->actionArg == 0) {
        play_mario_sound(m, SOUND_ACTION_THROW, get_mario_sound_id(MARIO_SND_HOOHOO));
    } else {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    }
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        if (m->actionTimer >= m->marioObj->header.gfx.animInfo.curAnimLogic->loopEnd + 4) {
            play_sound(get_mario_sound_id(MARIO_SND_GPWAH), m->marioObj->header.gfx.cameraToObject);
...
s32 act_thrown_backward(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_WAAAOOOW), MARIO_MARIO_SOUND_PLAYED);
...
#endif
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_ON_FIRE), MARIO_MARIO_SOUND_PLAYED);
#if ENABLE_RUMBLE
        queue_rumble_data(5, 80);
    }
#endif
...
s32 act_jump_kick(struct MarioState *m) {
    if (m->actionState == ACT_STATE_JUMP_KICK_PLAY_SOUND_AND_ANIM) {
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_HOO), MARIO_ACTION_SOUND_PLAYED);
...
s32 act_flying(struct MarioState *m) {
    ...
    if (startPitch <= 0 && m->faceAngle[0] > 0 && m->forwardVel >= 48.0f) {
        ...
        play_sound(get_mario_sound_id(MARIO_SND_YAHOO_WAHA_YIPPEE) + ((gAudioRandom % 5) << 16),
                   m->marioObj->header.gfx.cameraToObject);
#if ENABLE_RUMBLE
...
s32 act_vertical_wind(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_HERE_WE_GO), MARIO_MARIO_SOUND_PLAYED);
