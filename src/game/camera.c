...
void update_camera(struct Camera *c) {
    ...
    if (c->cutscene == CUTSCENE_NONE
        ...
        if (cam_select_alt_mode(CAM_SELECTION_NONE) == CAM_SELECTION_MARIO) {
            if (!gModuleMenuOpen && (gPlayer1Controller->buttonPressed & R_TRIG)) {
 ...
 */
struct CameraTrigger sCamTemple[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamRogue[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamPitstop[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamRf[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamFinalboss[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamWin[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamGameover[] = {
	NULL_TRIGGER
};
