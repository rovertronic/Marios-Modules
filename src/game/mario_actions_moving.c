...
void play_step_sound(struct MarioState *m, s16 frame1, s16 frame2) {
    if (is_anim_past_frame(m, frame1) || is_anim_past_frame(m, frame2)) {
        if (m->floor && (m->floor->type == SURFACE_VANISH_CAP_WALLS || m->floor->type == SURFACE_METAL_SOUND )) {
            play_sound(SOUND_ACTION_METAL_STEP, m->marioObj->header.gfx.cameraToObject);
        } else if (m->flags & MARIO_METAL_CAP) {
    ...
    switch (perform_ground_step(m)) {
        case GROUND_STEP_LEFT_GROUND:
            ...
            if (m->forwardVel < -50.0f || 50.0f < m->forwardVel) {
                play_sound(get_mario_sound_id(MARIO_SND_HOOHOO), m->marioObj->header.gfx.cameraToObject);
            }
            break;
...
s32 common_ground_knockback_action(struct MarioState *m, s32 animation, s32 checkFrame, s32 playLandingSound, s32 actionArg) {
    ...
    if (actionArg > 0) {
        ...
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_OOOF2), MARIO_MARIO_SOUND_PLAYED);
    }
...
s32 act_triple_jump_land(struct MarioState *m) {
    ...
    if (!(m->input & INPUT_NONZERO_ANALOG)) {
        play_sound_if_no_flag(m, get_mario_sound_id(MARIO_SND_HAHA), MARIO_MARIO_SOUND_PLAYED);
    }
