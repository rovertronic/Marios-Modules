...
s8 lootTableVanity[] = {MOD_VAN_CAP,MOD_VAN_PANTS,MOD_VAN_HAIR,MOD_RED,MOD_BLUE,MOD_GREEN,MOD_YELLOW,MOD_BLACK,MOD_WHITE,MOD_NO_CAP};
// 2x jump, 1x upg+1, 1x ground upg, 1x attack, 1x ground, 1x wall, 2x timer, 2x input, 1x heat sink, 1x down, 5x if conditions, 1x rotate, 1x time extend, 1x cancel, 1x crouch
s8 lootTableTier1[] = {MOD_JUMP, MOD_JUMP, MOD_POW, MOD_GROUND_UPG, MOD_ATTACK, MOD_HIT_GROUND, MOD_HIT_WALL, MOD_TIMER, MOD_TIMER, MOD_INPUT, MOD_INPUT, MOD_COOL, MOD_GRAV, MOD_IF_FLOOR, MOD_IF_DOWN, MOD_IF_INPUT, MOD_IF_SENSOR, MOD_ROTATE, MOD_TIME_EXTEND, MOD_CANCEL, MOD_CROUCH};
